...
pageheader:
  left: "WSESE 2025"
  right: 'Torchiano et al.: Exploring How SE Academic Papers Use Figures'

citation:
  type: paper-conference
  citation-key: torchianofigse2025
...
errors_tab |> kableExtra::kbl(format="latex",booktabs = T, align="lp{0.8\\linewidth}")
```
:::
::: {.content-hidden unless-format="html"}
A summary of the errors we identified, divided by severity is reported in table @tbl-errors_html.

```{r}
#| label: tbl-errors_html
#| tbl-cap: "Summary of errors by severity level."

errors_tab |> kableExtra::kbl(booktabs = T, align="ll")
```
:::
...
- **RQ1**. Mode: how are figures used in SE articles?
  ...
  To have an initial assessment of the phenomenon we consider important to understand how many figures are used in SE papers.
  Also it is interesting to observe if there is a trend in time concerning the usage of figures.
...
graph_types_tab |> kableExtra::kbl(format="latex",booktabs = T, align="lp{0.8\\linewidth}")
```
:::
::: {.content-hidden unless-format="html"}
Concerning the figures, in addition to the main distinction between quantitative (Q) and non-quantitative (NonQ), we categorized the type of content.
The list of types of figure contents are reported in @tbl-types_html divided into quantitative and non quantitative.

```{r}
#| label: tbl-types_html
#| tbl-cap: "Types of figures"

graph_types_tab |> kableExtra::kbl(booktabs = T, align="ll")
```
:::
...
For each article we went though it and for each figure we classified it using the taxonomy presented in the section above.
...
Since the length of the articles can be quite diverse, we computed a derived measure that is the density of figures, i.e. the number of figures per page.
...
In the taxonomy we used for quantitative diagram types -- reported in @sec-variables --, we have different types of bar plots.
