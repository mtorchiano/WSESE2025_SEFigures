...
As a practitioner who regularly writes papers and supervises younger researchers, I often find myself providing guidance on the effective use of visualizations. This necessity arises from observing a recurring issue: poorly designed diagrams that fail to effectively convey the intended information. Additionally, as a reviewer, I frequently encounter diagrams that are not only visually unappealing but also ineffective in communicating complex ideas. These experiences underscore a broader concern within the community regarding the presentation quality of SE papers. A critical question emerges: is it worthwhile to invest substantial effort in designing more sophisticated and effective diagrams? This paper aims to address this question by examining the current state of visualization practices in SE research.
...
- Recommendations: Based on our analysis, we propose a set of recommendations aimed at helping authors avoid common pitfalls and enhance the effectiveness of their visualizations.

Overall by surveying the current practices and offering practical guidance, this paper seeks to elevate the standard of visual communication in software engineering research, ultimately contributing to the clarity and impact of published work.
...
- **RQ2**. Content: what are figures used for?
  ...
  In addition it is interesting to understand what figures are used to show.


- **RQ3**. Type: what types of diagrams are used to convey quantitative information?

  Focusing on quantitative diagrams we wonder the type of diagrams used 
  in the papers.

- **RQ4**. Mistakes: What are the errors committed in quantitative diagrams?

  We focus on all diagrams and the we analyze the specific mistakes
...
The list of types of figure contents are reported in table @tbl-types divided into quantitative and non quantitative.
...
We downloaded all the articles in 2022 issues of the two journals, those appearing in the year 2017, 2019, 2021, and 2022 of ESEM and a sample of those appearing in years 2018, 2019, 2021, and 2022 of ICSE. 
...
if(FALSE){
# Checks
dg|> filter(is.na(VenueType))

dg %>% group_by(Venue,Year) %>%
  summarize(Articles = length(unique(DOI)), Figures=n(), .groups="drop") %>%
  add_row(.,Venue="Total",Articles=sum(.$Articles),Figures=sum(.$Figures)) %>%
  knitr::kable()

files = read_csv("all_files.csv")
dg |> filter(str_detect(Type,"\\?")) |> select(Venue,Year,DOI,Type) |>
  left_join(files |> select(DOI,file),by="DOI") |> mutate(file = str_sub(file,end=80))

dg |> select(main_type,Type) |> 
  mutate(Type = str_to_lower(Type)) |> 
  arrange(main_type,Type) |> 
  group_by(main_type,Type) |> summarize(n=n()) |>
  distinct() |> 
  unfill("main_type")

dg |> full_join(papers, by = join_by(DOI)) |> filter(is.na(Venue.x)) |> count(Venue.y,Year.y) |> filter(Venue.y=="ICSE")
dg |> full_join(papers, by = join_by(DOI)) |> filter(is.na(Venue.x)) |> filter(Venue.y=="ICSE")
}
```
...
if(FALSE){
  dg %>% 
  group_by(Venue,DOI) %>% 
  summarize(num_fig=sum(Fig!=0),fig_per_page=num_fig/pages,.groups="drop") |>
  summarize( WithoutFigure=sum(num_fig==0), 
             WithoutFigurePct=paste0(round(WithoutFigure/n()*100,1),"%"), 
             MeanFigurePP=round(mean(fig_per_page),1), 
             SDFigurePP=round(sd(fig_per_page),1),
             Q1 = quantile(fig_per_page,0.25),
             Q3 = quantile(fig_per_page,0.75))
}

```

By looking at the figure we can observe a significant differences among the four venues. 
...
Overall one every three quantitative figures contains a bar plot The two other common used plot types are boxplots (19.6%) and line plots (18.7%).
...
We observe the average number of errors per figure is 1.6. Overall half of the figures exhibit at least one error, with half the EMSE articles showing at least 2 errors. Concerning the proportion of figures where no error was detected, they are `r NoErrorSummary`.
...
As far as minor errors are concerned, we observe a similar amount of defective figures for all venues except EMSE where the 60% of figures contain at least one error.
...
if(FALSE){
dg %>% filter(quantitative) %>% 
  pivot_longer(cols=all_of(errors$Error),names_to = "Error") %>% 
  group_by(Error) %>% 
  summarize( n = sum(value), .groups = "drop") |>
  full_join(errors |> select(Error,Severity),by="Error") |> arrange(Severity,n)
}
...
The two most common minor errors regard the use of legends, often (20.1% of figures) the legend is placed inside the plot are affecting the clarity, moreover the legend is surrounded by a border (17.8%) that affects the ease of scanning back and forth between plot and legend itself.
...
As a comparison the present paper sports 12 figures in 8 pages, that is 1.5 figures per page, a clear outlier if we look at @fig-density.
...
Talking about errors, we found 1.6 errors per figure on average. Anyway, across all venues, one in three figures showed no errors according to our taxonomy, while considering only critical errors, three out of four figures are fine. Focusing on major and minor errors, we observed that half of the figures showed at least one.
...
The lack of values on the axes is another relatively common error that has an heavy impact on the ability to understand the diagram.
 Often in graphs generated by basic tools -- e.g. spreadsheet programs -- the categories are automatically encoded using many different variation of a single visual attribute -- e.g. shape or color -- that require a lot of cognitive effort to be discerned. A little more design effort should be spent in finding alternative representations that make understanding more immediate.
Another common result of using unaltered graphs produced by, e.g. spreadsheets, is the having rotated or slanted labels on the x axis, making more difficult than required reading the graph. Most of the time a simple solution is to swap x and y axes; this should be the immediate choice in presence of long labels.
One other common mistake that is worth mentioning is the use of a legend instead of direct labelling the series of data in the plot area. This is probably due to the default in most tools as well as the difficulty in implementing direct labelling in those tools.
...
considered. Although the venues have a very good reputation, they do not represent the whole publication spectrum in SE. Moreover the articles a drawn from a limited time span, they represent a sample. This might affect the external validity of the study.
The graph error taxonomy has been built based on the literature in the area of data visualization, but it could have omitted important errors or, vice-versa, considered error that are not such in the view of other researchers. In addition the severity level has been assigned to each error on the basis of the author expert judgment.
...
We found the average density if 0.42 figures per page and half of them are used to convey quantitative information.
...
As future work we would like to extend the survey to a wider time span as well as other venues. In addition we should take into consideration figures with multiple types. Eventually, a pragmatic guide to assess and improve quantitive diagrams could be defined based on this work.
